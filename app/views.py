...
def show_uploads(request):
    ...
    html = ["<html><body><h1>Uploads</h1><ul>"]
    for up in uploads:
        html.append(
            "<li>"
            f"{up.uploaded_by.username} | "
            f"{up.institution.name} | "
            f"{up.reporting_year.label} | "
            f"{os.path.basename(up.file.name) if up.file else 'no file'} | "
            f'<a href="/app/api/download/{up.id}/">download</a> | '
            f'<a href="/app/api/process/{up.id}/">process</a>'
            "</li>"
        )
